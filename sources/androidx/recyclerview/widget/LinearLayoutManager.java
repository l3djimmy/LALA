...
public class LinearLayoutManager extends RecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    public boolean performAccessibilityAction(int action, Bundle args) {
        ...
        if (action == 16908343 && args != null) {
            if (this.mOrientation == 1) {
                int rowArg = args.getInt(AccessibilityNodeInfoCompat.ACTION_ARGUMENT_ROW_INT, -1);
                if (rowArg < 0) {
                    return false;
                }
                position = Math.min(rowArg, getRowCountForAccessibility(this.mRecyclerView.mRecycler, this.mRecyclerView.mState) - 1);
            } else {
                int columnArg = args.getInt(AccessibilityNodeInfoCompat.ACTION_ARGUMENT_COLUMN_INT, -1);
                if (columnArg < 0) {
                    return false;
                }
                position = Math.min(columnArg, getColumnCountForAccessibility(this.mRecyclerView.mRecycler, this.mRecyclerView.mState) - 1);
            }
            if (position >= 0) {
