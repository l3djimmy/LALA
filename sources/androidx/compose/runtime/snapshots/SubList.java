...
public final class SubList<T> implements List<T>, KMutableList {
    ...
    public boolean addAll(int index, Collection<? extends T> collection) {
        validateModification();
        boolean result = this.parentList.addAll(this.offset + index, collection);
        if (result) {
    ...
    public boolean retainAll(Collection<? extends Object> collection) {
        validateModification();
        int removed = this.parentList.retainAllInRange$runtime_release(collection, this.offset, this.offset + size());
        if (removed > 0) {
            this.structure = this.parentList.getStructure$runtime_release();
