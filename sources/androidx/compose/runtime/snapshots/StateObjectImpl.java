...
public abstract class StateObjectImpl implements StateObject {
    ...
    public final void m3864recordReadInh_f27i8$runtime_release(int reader) {
        int old;
        do {
            old = ReaderKind.m3851constructorimpl(this.readerKind.get());
            if ((old & reader) != 0) {
                return;
            }
        } while (!this.readerKind.compareAndSet(old, ReaderKind.m3851constructorimpl(old | reader)));
    }

    /* renamed from: isReadIn-h_f27i8$runtime_release  reason: not valid java name */
    public final boolean m3863isReadInh_f27i8$runtime_release(int reader) {
        int arg0$iv = ReaderKind.m3851constructorimpl(this.readerKind.get());
