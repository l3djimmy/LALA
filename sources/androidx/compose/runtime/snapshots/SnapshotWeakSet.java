...
public final class SnapshotWeakSet<T> {
    ...
    public final boolean add(T t) {
        int index;
        int size = this.size;
        int hash = Utils_jvmKt.identityHashCode(t);
        if (size > 0) {
            index = find(t, hash);
            if (index >= 0) {
                return false;
    ...
    public final void removeIf(Function1<? super T, Boolean> function1) {
        int size = getSize$runtime_release();
        int currentUsed = 0;
        int i = 0;
        while (true) {
            ...
            if (entry != null) {
                value = (Object) entry.get();
            }
            if (value != null && !function1.invoke(value).booleanValue()) {
                if (currentUsed != i) {
    ...
    private final int find(T t, int hash) {
        int low = 0;
        ...
        while (low <= high) {
            ...
            if (midHash < hash) {
                ...
                return t == midVal ? mid : findExactIndex(mid, t, hash);
            } else {
                high = mid - 1;
            }
        }
    ...
    private final int findExactIndex(int midIndex, T t, int valueHash) {
        int i = midIndex - 1;
        while (true) {
