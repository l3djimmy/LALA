...
public final class SmallPersistentVector<E> extends AbstractPersistentList<E> implements ImmutableList<E> {
    ...
    public SmallPersistentVector(Object[] buffer) {
        this.buffer = buffer;
        CommonFunctionsKt.m3844assert(this.buffer.length <= 32);
    }
    ...
    public PersistentList<E> addAll(Collection<? extends E> collection) {
        if (size() + collection.size() <= 32) {
            Object[] newBuffer = Arrays.copyOf(this.buffer, size() + collection.size());
            Intrinsics.checkNotNullExpressionValue(newBuffer, "copyOf(...)");
            int index = size();
    ...
    public PersistentList<E> addAll(int index, Collection<? extends E> collection) {
        ...
        if (size() + collection.size() <= 32) {
            Object[] newBuffer = bufferOfSize(size() + collection.size());
            ArraysKt.copyInto$default(this.buffer, newBuffer, 0, 0, index, 6, (Object) null);
            ArraysKt.copyInto(this.buffer, newBuffer, collection.size() + index, index, size());
            int position = index;
            for (Object element : collection) {
