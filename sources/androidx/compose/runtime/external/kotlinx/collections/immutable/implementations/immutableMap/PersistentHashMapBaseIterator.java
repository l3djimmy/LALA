...
public abstract class PersistentHashMapBaseIterator<K, V, T> implements Iterator<T>, KMappedMarker {
    ...
    public PersistentHashMapBaseIterator(TrieNode<K, V> trieNode, TrieNodeBaseIterator<K, V, T>[] trieNodeBaseIteratorArr) {
        ...
        this.path[0].reset(trieNode.getBuffer$runtime_release(), trieNode.entryCount$runtime_release() * 2);
    ...
    private final int moveToNextNodeWithData(int pathIndex) {
        ...
        if (this.path[pathIndex].hasNextNode()) {
            ...
            if (pathIndex == 6) {
                trieNodeBaseIteratorArr[pathIndex + 1].reset(node.getBuffer$runtime_release(), node.getBuffer$runtime_release().length);
            } else {
                trieNodeBaseIteratorArr[pathIndex + 1].reset(node.getBuffer$runtime_release(), node.entryCount$runtime_release() * 2);
            }
