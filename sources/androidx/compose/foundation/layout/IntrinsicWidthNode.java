...
final class IntrinsicWidthNode extends IntrinsicSizeModifier {
    ...
    public long mo946calculateContentConstraintsl58MMJ0(MeasureScope $this$calculateContentConstraints_u2dl58MMJ0, Measurable measurable, long constraints) {
        ...
        if (this.width == IntrinsicSize.Min) {
            measuredWidth = measurable.minIntrinsicWidth(Constraints.m6825getMaxHeightimpl(constraints));
        } else {
            measuredWidth = measurable.maxIntrinsicWidth(Constraints.m6825getMaxHeightimpl(constraints));
        }
        if (measuredWidth < 0) {
            measuredWidth = 0;
        }
        return Constraints.Companion.m6838fixedWidthOenEA2s(measuredWidth);
    }
