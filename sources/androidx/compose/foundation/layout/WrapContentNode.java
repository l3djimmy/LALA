...
final class WrapContentNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo328measure3p2s80s(final MeasureScope $this$measure_u2d3p2s80s, Measurable measurable, long constraints) {
        int m6826getMaxWidthimpl;
        int m6828getMinWidthimpl = this.direction != Direction.Vertical ? 0 : Constraints.m6828getMinWidthimpl(constraints);
        int m6827getMinHeightimpl = this.direction == Direction.Horizontal ? Constraints.m6827getMinHeightimpl(constraints) : 0;
        int i = Integer.MAX_VALUE;
        if (this.direction != Direction.Vertical && this.unbounded) {
            m6826getMaxWidthimpl = Integer.MAX_VALUE;
        } else {
            m6826getMaxWidthimpl = Constraints.m6826getMaxWidthimpl(constraints);
        }
        if (this.direction == Direction.Horizontal || !this.unbounded) {
            i = Constraints.m6825getMaxHeightimpl(constraints);
        }
        long wrappedConstraints = ConstraintsKt.Constraints(m6828getMinWidthimpl, m6826getMaxWidthimpl, m6827getMinHeightimpl, i);
        final Placeable placeable = measurable.mo5637measureBRTryo0(wrappedConstraints);
        final int wrapperWidth = RangesKt.coerceIn(placeable.getWidth(), Constraints.m6828getMinWidthimpl(constraints), Constraints.m6826getMaxWidthimpl(constraints));
        final int wrapperHeight = RangesKt.coerceIn(placeable.getHeight(), Constraints.m6827getMinHeightimpl(constraints), Constraints.m6825getMaxHeightimpl(constraints));
        return MeasureScope.layout$default($this$measure_u2d3p2s80s, wrapperWidth, wrapperHeight, null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.layout.WrapContentNode$measure$1
            ...
            public final void invoke2(Placeable.PlacementScope $this$layout) {
                Function2<IntSize, LayoutDirection, IntOffset> alignmentCallback = WrapContentNode.this.getAlignmentCallback();
                int width$iv = wrapperWidth - placeable.getWidth();
                int height$iv = wrapperHeight - placeable.getHeight();
                long position = alignmentCallback.invoke(IntSize.m7036boximpl(IntSize.m7039constructorimpl((width$iv << 32) | (height$iv & 4294967295L))), $this$measure_u2d3p2s80s.getLayoutDirection()).m7010unboximpl();
