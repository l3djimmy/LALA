...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long mo946calculateContentConstraintsl58MMJ0(MeasureScope $this$calculateContentConstraints_u2dl58MMJ0, Measurable measurable, long constraints) {
        int measuredHeight;
        if (this.height == IntrinsicSize.Min) {
            measuredHeight = measurable.minIntrinsicHeight(Constraints.m6826getMaxWidthimpl(constraints));
        } else {
            measuredHeight = measurable.maxIntrinsicHeight(Constraints.m6826getMaxWidthimpl(constraints));
        }
        if (measuredHeight < 0) {
            measuredHeight = 0;
        }
        return Constraints.Companion.m6837fixedHeightOenEA2s(measuredHeight);
    }
