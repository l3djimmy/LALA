...
public final class CalculateMatrixToWindowApi21 implements CalculateMatrixToWindow {
    ...
    public void mo6021calculateMatrixToWindowEL8BTi8(View view, float[] matrix) {
        Matrix.m4528resetimpl(matrix);
        m6024transformMatrixToWindowEL8BTi8(view, matrix);
    }

    /* renamed from: transformMatrixToWindow-EL8BTi8  reason: not valid java name */
    private final void m6024transformMatrixToWindowEL8BTi8(View view, float[] matrix) {
        ViewParent parentView = view.getParent();
        if (parentView instanceof View) {
            m6024transformMatrixToWindowEL8BTi8((View) parentView, matrix);
            m6023preTranslate3XD1CNM(matrix, -view.getScrollX(), -view.getScrollY());
            m6023preTranslate3XD1CNM(matrix, view.getLeft(), view.getTop());
        } else {
            int[] pos = this.tmpLocation;
            view.getLocationInWindow(pos);
            m6023preTranslate3XD1CNM(matrix, -view.getScrollX(), -view.getScrollY());
            m6023preTranslate3XD1CNM(matrix, pos[0], pos[1]);
        }
        android.graphics.Matrix viewMatrix = view.getMatrix();
        if (!viewMatrix.isIdentity()) {
            m6022preConcattUYjHk(matrix, viewMatrix);
        }
    }

    /* renamed from: preConcat-tU-YjHk  reason: not valid java name */
    private final void m6022preConcattUYjHk(float[] $this$preConcat_u2dtU_u2dYjHk, android.graphics.Matrix other) {
        AndroidMatrixConversions_androidKt.m4160setFromtUYjHk(this.tmpMatrix, other);
        AndroidComposeView_androidKt.m6019preTransformJiSxe2E($this$preConcat_u2dtU_u2dYjHk, this.tmpMatrix);
    }

    /* renamed from: preTranslate-3XD1CNM  reason: not valid java name */
    private final void m6023preTranslate3XD1CNM(float[] $this$preTranslate_u2d3XD1CNM, float x, float y) {
        AndroidComposeView_androidKt.m6020preTranslatecG2Xzmc($this$preTranslate_u2d3XD1CNM, x, y, this.tmpMatrix);
    }
}
