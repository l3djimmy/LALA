...
public class FontRequestWorker {
    ...
    public static Typeface requestFontSync(final Context context, final FontRequest request, CallbackWrapper callback, final int style, int timeoutInMillis) {
        ...
        final String id = createCacheId(m, style);
        Typeface cached = sTypefaceCache.get(id);
        if (cached != null) {
            callback.onTypefaceResult(new TypefaceResult(cached));
            return cached;
        } else if (timeoutInMillis == -1) {
            m2 = TypefaceCompat$$ExternalSyntheticBackport2.m(new Object[]{request});
            TypefaceResult typefaceResult = getFontSync(id, context, m2, style);
            ...
            Callable<TypefaceResult> fetcher = new Callable<TypefaceResult>() { // from class: androidx.core.provider.FontRequestWorker.1
                ...
                public TypefaceResult call() {
                    ...
                    m3 = TypefaceCompat$$ExternalSyntheticBackport2.m(new Object[]{request});
                    return FontRequestWorker.getFontSync(str, context2, m3, style);
                }
            };
            try {
