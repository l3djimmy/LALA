...
public class CamColor {
    ...
    int viewed(ViewingConditions viewingConditions) {
        float alpha;
        if (getChroma() == AudioStats.AUDIO_AMPLITUDE_NONE || getJ() == AudioStats.AUDIO_AMPLITUDE_NONE) {
            alpha = 0.0f;
        } else {
        ...
        float rCBase = (float) Math.max((double) AudioStats.AUDIO_AMPLITUDE_NONE, (Math.abs(rA) * 27.13d) / (400.0d - alpha2));
        float rC = ((float) Math.pow(rCBase, 2.380952380952381d)) * Math.signum(rA) * (100.0f / viewingConditions.getFl());
        float gCBase = (float) Math.max((double) AudioStats.AUDIO_AMPLITUDE_NONE, (Math.abs(gA) * 27.13d) / (400.0d - Math.abs(gA)));
        float gC = Math.signum(gA) * (100.0f / viewingConditions.getFl()) * ((float) Math.pow(gCBase, 2.380952380952381d));
        float bCBase = (float) Math.max((double) AudioStats.AUDIO_AMPLITUDE_NONE, (Math.abs(bA) * 27.13d) / (400.0d - Math.abs(bA)));
