...
public final class ComponentActivity$activityResultRegistry$1 extends ActivityResultRegistry {
    ...
    public <I, O> void onLaunch(final int requestCode, ActivityResultContract<I, O> contract, I i, ActivityOptionsCompat options) {
        Bundle optionsBundle;
        Intrinsics.checkNotNullParameter(contract, "contract");
        ComponentActivity activity = this.this$0;
        final ActivityResultContract.SynchronousResult synchronousResult = contract.getSynchronousResult(activity, i);
        ...
        Intent intent = contract.createIntent(activity, i);
        ...
        if (Intrinsics.areEqual(ActivityResultContracts.RequestMultiplePermissions.ACTION_REQUEST_PERMISSIONS, intent.getAction())) {
        ...
        } else if (Intrinsics.areEqual(ActivityResultContracts.StartIntentSenderForResult.ACTION_INTENT_SENDER_REQUEST, intent.getAction())) {
            ...
            try {
                ...
                new Handler(Looper.getMainLooper()).post(new Runnable() { // from class: androidx.activity.ComponentActivity$activityResultRegistry$1$$ExternalSyntheticLambda1
                    @Override // java.lang.Runnable
                    public final void run() {
                        ComponentActivity$activityResultRegistry$1.this.dispatchResult(requestCode, 0, new Intent().setAction(ActivityResultContracts.StartIntentSenderForResult.ACTION_INTENT_SENDER_REQUEST).putExtra(ActivityResultContracts.StartIntentSenderForResult.EXTRA_SEND_INTENT_EXCEPTION, e));
                    }
                });
            }
        } else {
