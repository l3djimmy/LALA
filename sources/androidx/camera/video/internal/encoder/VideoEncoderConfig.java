...
public abstract class VideoEncoderConfig implements EncoderConfig {
    ...
    public MediaFormat toMediaFormat() {
        ...
        MediaFormat format = MediaFormat.createVideoFormat(getMimeType(), size.getWidth(), size.getHeight());
        format.setInteger("color-format", getColorFormat());
        format.setInteger("bitrate", getBitrate());
        format.setInteger("frame-rate", getFrameRate());
        format.setInteger("i-frame-interval", getIFrameInterval());
        if (getProfile() != -1) {
            format.setInteger(Scopes.PROFILE, getProfile());
        }
