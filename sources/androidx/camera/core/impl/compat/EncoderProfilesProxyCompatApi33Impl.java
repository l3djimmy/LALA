...
class EncoderProfilesProxyCompatApi33Impl {
    ...
    private static List<EncoderProfilesProxy.VideoProfileProxy> fromVideoProfiles(List<EncoderProfiles.VideoProfile> profiles) {
        ...
        for (EncoderProfiles.VideoProfile profile : profiles) {
            proxies.add(EncoderProfilesProxy.VideoProfileProxy.create(profile.getCodec(), profile.getMediaType(), profile.getBitrate(), profile.getFrameRate(), profile.getWidth(), profile.getHeight(), profile.getProfile(), profile.getBitDepth(), profile.getChromaSubsampling(), profile.getHdrFormat()));
        }
        return proxies;
    }
