...
final class Image2JpegBytes implements Operation<In, Packet<byte[]>> {
    ...
    public Packet<byte[]> apply(In input) throws ImageCaptureException {
        try {
    ...
    private Packet<byte[]> processJpegImage(In input) {
    ...
    private Packet<byte[]> processYuvImage(In input) throws ImageCaptureException {
        Packet<ImageProxy> packet = input.getPacket();
        ImageProxy image = packet.getData();
        Rect cropRect = packet.getCropRect();
        try {
            byte[] jpegBytes = ImageUtil.yuvImageToJpegByteArray(image, cropRect, input.getJpegQuality(), packet.getRotationDegrees());
            return Packet.of(jpegBytes, extractExif(jpegBytes), 256, new Size(cropRect.width(), cropRect.height()), new Rect(0, 0, cropRect.width(), cropRect.height()), packet.getRotationDegrees(), TransformUtils.updateSensorToBufferTransform(packet.getSensorToBufferTransform(), cropRect), packet.getCameraCaptureResult());
        } catch (ImageUtil.CodecFailedException e) {
            throw new ImageCaptureException(1, "Failed to encode the image to JPEG.", e);
        }
    }

    private static Exif extractExif(byte[] jpegBytes) throws ImageCaptureException {
        try {
    ...
    public static abstract class In {
        ...
        /* JADX INFO: Access modifiers changed from: package-private */
        public static In of(Packet<ImageProxy> imagePacket, int jpegQuality) {
