...
public class MonotonicCurveFit extends CurveFit {
    ...
    public MonotonicCurveFit(double[] time, double[][] y) {
        ...
        for (int i3 = 0; i3 < n - 1; i3++) {
            for (int j2 = 0; j2 < dim; j2++) {
                if (slope[i3][j2] == AudioStats.AUDIO_AMPLITUDE_NONE) {
                    tangent[i3][j2] = 0.0d;
        ...
        for (int i2 = 0; i2 < n - 1; i2++) {
            ...
            if (t < this.mT[i2 + 1]) {
                ...
                for (int j6 = 0; j6 < dim; j6++) {
                    ...
                    double t1 = this.mTangent[i2][j6];
                    double t2 = this.mTangent[i2 + 1][j6];
    ...
    public void getSlope(double t, double[] v) {
        ...
        for (int i2 = 0; i2 < n - 1; i2++) {
            if (t2 <= this.mT[i2 + 1]) {
                double h = this.mT[i2 + 1] - this.mT[i2];
                double x = (t2 - this.mT[i2]) / h;
                for (int j = 0; j < dim; j++) {
                    double y1 = this.mY[i2][j];
                    double y2 = this.mY[i2 + 1][j];
                    double t1 = this.mTangent[i2][j];
                    double t22 = this.mTangent[i2 + 1][j];
