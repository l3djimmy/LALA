...
public class ConstraintLayoutStatistics {
    ...
    /* renamed from: clone */
    public ConstraintLayoutStatistics m7136clone() {
    ...
    private String log(DecimalFormat df, int param) {
        String value = fmt(df, ((float) getValue(param)) * 1.0E-6f, 7);
    ...
    private String compare(DecimalFormat df, ConstraintLayoutStatistics relative, int param) {
        String value = fmt(df, ((float) getValue(param)) * 1.0E-6f, 7);
        String title = WORD_PAD + geName(param);
        return "CL Perf: " + (title.substring(title.length() - MAX_WORD) + " = ") + (value + " -> " + fmt(df, ((float) relative.getValue(param)) * 1.0E-6f, 7) + "ms");
    }
