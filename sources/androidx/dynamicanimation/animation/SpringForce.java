...
public final class SpringForce implements Force {
    ...
    private void init() {
        ...
        int i = (this.mDampingRatio > 1.0d ? 1 : (this.mDampingRatio == 1.0d ? 0 : -1));
        double d = this.mDampingRatio;
        if (i > 0) {
            this.mGammaPlus = ((-d) * this.mNaturalFreq) + (this.mNaturalFreq * Math.sqrt((this.mDampingRatio * this.mDampingRatio) - 1.0d));
        ...
        } else if (d >= AudioStats.AUDIO_AMPLITUDE_NONE && this.mDampingRatio < 1.0d) {
    ...
    public DynamicAnimation.MassState updateValues(double lastDisplacement, double lastVelocity, long timeElapsed) {
        ...
        if (this.mDampingRatio > 1.0d) {
            ...
            currentVelocity = d + (pow * (((-this.mDampedFreq) * lastDisplacement2 * Math.sin(cosCoeff * deltaT)) + (Math.cos(this.mDampedFreq * deltaT) * this.mDampedFreq * sinCoeff)));
        } else {
