...
public class Image extends AbstractElement<Image> implements ILeafElement, IAccessibleElement {
    ...
    public float getImageScaledWidth() {
        Object property = getProperty(29);
        PdfXObject pdfXObject = this.xObject;
        if (property == null) {
            return pdfXObject.getWidth();
        }
        return ((Float) getProperty(29)).floatValue() * pdfXObject.getWidth();
    }

    public float getImageScaledHeight() {
        Object property = getProperty(76);
        PdfXObject pdfXObject = this.xObject;
        if (property == null) {
            return pdfXObject.getHeight();
        }
        return ((Float) getProperty(76)).floatValue() * pdfXObject.getHeight();
    }
    ...
    public ObjectFit getObjectFit() {
        if (hasProperty(Property.OBJECT_FIT)) {
            return (ObjectFit) getProperty(Property.OBJECT_FIT);
        }
