...
public class HyphenationTree extends TernaryTree implements IPatternConsumer {
    ...
    private List<char[]> splitOnNonCharacters(char[] word) {
        ...
        int ibreak2 = word.length;
        if ((ibreak2 - breakPoints.get(breakPoints.size() - 1).intValue()) - 1 > 1) {
    ...
    private int[] getHyphPointsForWords(List<char[]> nonLetterWords, int remainCharCount, int pushCharCount) {
        ...
        while (iNonLetterWord < nonLetterWords.size()) {
            ...
            if (curHyph != null) {
                ...
                int i2 = breaks.length;
                System.arraycopy(breaks, 0, combined, 0, i2);
            ...
            if (i2 <= len) {
                ...
                if (this.stoplist.containsKey(sw)) {
                    ...
                    while (true) {
                        ...
                        Object o = hw.get(i4);
                        char[] c2 = c;
                        if ((o instanceof String) && (j = j + ((String) o).length()) >= remainCharCount && j < len2 - pushCharCount) {
                    ...
                    for (int i6 = 0; i6 < len2; i6++) {
                        if ((il[i6 + 1] & 1) == 1 && i6 >= remainCharCount && i6 <= len2 - pushCharCount) {
                            result[k] = i6;
                            k++;
                        }
                    }
                ...
                if (k > 0) {
                    int[] res = new int[k];
                    int i7 = i;
                    System.arraycopy(result, i7, res, i7, k);
