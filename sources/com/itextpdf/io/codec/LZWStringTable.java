...
public class LZWStringTable {
    ...
    public int AddCharString(short index, byte b) {
        ...
        short[] sArr = this.strNxt_;
        if (index == -1) {
            sArr[this.numStrings_] = -1;
            this.strLen_[this.numStrings_] = 1;
        } else {
            sArr[this.numStrings_] = index;
    ...
    public void dump(PrintStream output) {
        for (int i = 258; i < this.numStrings_; i++) {
            output.println(" strNxt_[" + i + "] = " + ((int) this.strNxt_[i]) + " strChr_ " + Integer.toHexString(this.strChr_[i] & UByte.MAX_VALUE) + " strLen_ " + Integer.toHexString(this.strLen_[i]));
        }
    }
}
