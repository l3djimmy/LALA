...
public class Subpath {
    ...
    public List<Point> getPiecewiseLinearApproximation() {
        ...
        boolean z = this.segments.get(0) instanceof BezierCurve;
        List<IShape> list = this.segments;
        if (z) {
            result.addAll(((BezierCurve) list.get(0)).getPiecewiseLinearApproximation());
        } else {
            result.addAll(list.get(0).getBasePoints());
        }
        for (int i = 1; i < this.segments.size(); i++) {
            boolean z2 = this.segments.get(i) instanceof BezierCurve;
            List<IShape> list2 = this.segments;
            if (z2) {
                List<Point> segApprox2 = ((BezierCurve) list2.get(i)).getPiecewiseLinearApproximation();
                segApprox = segApprox2.subList(1, segApprox2.size());
            } else {
                List<Point> segApprox3 = list2.get(i).getBasePoints();
