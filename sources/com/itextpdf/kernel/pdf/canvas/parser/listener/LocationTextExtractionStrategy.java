        ...
        for (TextChunk current : toSort) {
            ...
            if (currentMarks2 != null) {
                if (this.rightToLeftRunDirection) {
                    for (int j = currentMarks2.succeeding.size() - 1; j >= 0; j--) {
                        textChunks.add(currentMarks2.succeeding.get(j));
                    }
                } else {
                    for (int j2 = 0; j2 < currentMarks2.preceding.size(); j2++) {
                        textChunks.add(currentMarks2.preceding.get(j2));
                    }
                }
            }
            ...
            if (currentMarks2 != null) {
                if (this.rightToLeftRunDirection) {
                    for (int j3 = currentMarks2.preceding.size() - 1; j3 >= 0; j3--) {
                        textChunks.add(currentMarks2.preceding.get(j3));
                    }
                } else {
                    for (int j4 = 0; j4 < currentMarks2.succeeding.size(); j4++) {
                        textChunks.add(currentMarks2.succeeding.get(j4));
                    }
                }
            }
        }
