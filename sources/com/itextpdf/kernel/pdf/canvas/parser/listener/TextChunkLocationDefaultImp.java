...
class TextChunkLocationDefaultImp implements ITextChunkLocation {
    ...
    public TextChunkLocationDefaultImp(Vector startLocation, Vector endLocation, float charSpaceWidth) {
        ...
        this.orientationMagnitude = (int) FontProgram.convertGlyphSpaceToTextSpace(Math.atan2(this.orientationVector.get(1), this.orientationVector.get(0)));
    ...
    public boolean sameLine(ITextChunkLocation as) {
        ...
        if (Math.abs(distPerpendicularDiff) <= DIACRITICAL_MARKS_ALLOWED_VERTICAL_DEVIATION) {
            return mySegment.getLength() == 0.0f || otherSegment.getLength() == 0.0f;
        }
        return false;
    }
