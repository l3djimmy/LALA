...
public class PdfStructElem extends PdfObjectWrapper<PdfDictionary> implements IStructureNode {
    ...
    public void setStructureElementId(PdfString id) {
        ...
        PdfObject orig2 = getPdfObject().get(PdfName.ID);
        if (id.equals(orig2)) {
            return;
        }
        if (orig2 instanceof PdfString) {
            idTree.removeEntry((PdfString) orig2);
        }
        idTree.addEntry(id, getPdfObject());
    ...
    public static void addKidObject(PdfDictionary parent, int index, PdfObject kid) {
        PdfArray a;
        if (parent.isFlushed()) {
    ...
    private IStructureNode convertPdfObjectToIPdfStructElem(PdfObject obj) {
        switch (obj.getType()) {
            case 3:
                ...
                if (isStructElem(d)) {
                    IStructureNode elem = new PdfStructElem(d);
                    return elem;
                } else if (PdfName.MCR.equals(d.getAsName(PdfName.Type))) {
                    IStructureNode elem2 = new PdfMcrDictionary(d, this);
                    return elem2;
                } else if (!PdfName.OBJR.equals(d.getAsName(PdfName.Type))) {
                    return null;
                } else {
                    IStructureNode elem3 = new PdfObjRef(d, this);
                    return elem3;
                }
            case 8:
                IStructureNode elem4 = new PdfMcrNumber((PdfNumber) obj, this);
                return elem4;
            default:
                return null;
        }
