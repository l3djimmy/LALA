...
public class PdfStructTreeRoot extends PdfObjectWrapper<PdfDictionary> implements IStructureNode {
    ...
    public void addRoleMapping(String fromRole, String toRole) {
        ...
        if (prevVal != null && (prevVal instanceof PdfName)) {
            ...
            logger.warn(MessageFormat.format(IoLogMessageConstant.MAPPING_IN_STRUCT_ROOT_OVERWRITTEN, fromRole, prevVal, toRole));
        }
    ...
    public PdfDictionary getRoleMap() {
        ...
        if (roleMap == null) {
            ...
            getPdfObject().put(PdfName.RoleMap, roleMap2);
            setModified();
            return roleMap2;
        }
        return roleMap;
    ...
    public PdfArray getNamespacesObject() {
        ...
        if (namespacesArray == null) {
            ...
            getPdfObject().put(PdfName.Namespaces, namespacesArray2);
            setModified();
    ...
    public void move(PdfPage fromPage, int insertBeforePage) {
        for (int i = 1; i <= getDocument().getNumberOfPages(); i++) {
            if (getDocument().getPage(i).isFlushed()) {
                throw new PdfException(MessageFormatUtil.format(KernelExceptionMessageConstant.CANNOT_MOVE_PAGES_IN_PARTLY_FLUSHED_DOCUMENT, Integer.valueOf(i)));
            }
        }
    ...
    public PdfStructIdTree getIdTree() {
        if (this.idTree == null) {
            PdfDictionary idTreeDict = getPdfObject().getAsDictionary(PdfName.IDTree);
            if (idTreeDict == null) {
                this.idTree = new PdfStructIdTree(this.document);
            } else {
                this.idTree = PdfStructIdTree.readFromDictionary(this.document, idTreeDict);
            }
        }
        return this.idTree;
    }
