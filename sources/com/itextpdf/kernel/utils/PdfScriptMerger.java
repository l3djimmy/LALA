...
public class PdfScriptMerger {
    ...
    public static void mergeOpenActionsScripts(PdfDocument source, PdfDocument destination) {
        PdfDictionary sourceOpenActionDict;
        PdfObject sourceOpenAction = source.getCatalog().getPdfObject().get(PdfName.OpenAction);
        if ((sourceOpenAction instanceof PdfArray) || (sourceOpenActionDict = source.getCatalog().getPdfObject().getAsDictionary(PdfName.OpenAction)) == null || sourceOpenActionDict.isEmpty() || !PdfName.JavaScript.equals(sourceOpenActionDict.get(PdfName.S))) {
            return;
        }
    ...
    public static void mergeNamesScripts(PdfDocument source, PdfDocument destination) {
        ...
        for (Map.Entry<PdfString, PdfObject> entry : sourceTree.getNames().entrySet()) {
            PdfDictionary ECMAScriptActionsDirectCopy = copyECMAScriptActionsDictionary(destination, (PdfDictionary) (entry.getValue().isIndirect() ? entry.getValue().getIndirectReference().getRefersTo() : entry.getValue()));
