...
public class DocTrueTypeFont extends TrueTypeFont implements IDocFontProgram {
    ...
    public static TrueTypeFont createFontProgram(PdfDictionary fontDictionary, FontEncoding fontEncoding, CMapToUnicode toUnicode) {
    ...
    static int getDefaultWithOfFont(PdfDictionary fontDictionary, PdfDictionary fontDescriptor) {
        if (fontDescriptor != null && fontDescriptor.containsKey(PdfName.DW)) {
            int defaultWidth = fontDescriptor.getAsInt(PdfName.DW).intValue();
            return defaultWidth;
        } else if (fontDictionary.containsKey(PdfName.DW)) {
            int defaultWidth2 = fontDictionary.getAsInt(PdfName.DW).intValue();
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public static TrueTypeFont createFontProgram(PdfDictionary fontDictionary, CMapToUnicode toUnicode) {
        DocTrueTypeFont fontProgram = new DocTrueTypeFont(fontDictionary);
        ...
        if (toUnicode != null) {
            ...
            fontProgram.avgWidth = 0;
            for (Integer num : toUnicode.getCodes()) {
        ...
        if (fontProgram.codeToGlyph.get(0) == null) {
            fontProgram.codeToGlyph.put(0, new Glyph(0, (widths == null || !widths.containsKey(0)) ? defaultWidth : widths.get(0), -1));
        }
        return fontProgram;
    }
