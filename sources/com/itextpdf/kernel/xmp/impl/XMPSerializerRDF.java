...
public class XMPSerializerRDF {
    ...
    protected void checkOptionsConsistence() throws XMPException {
        ...
        boolean exactPacketLength = this.options.getExactPacketLength();
        SerializeOptions serializeOptions = this.options;
        if (exactPacketLength) {
            if (serializeOptions.getOmitPacketWrapper() | this.options.getIncludeThumbnailPad()) {
        ...
        boolean readOnlyPacket = serializeOptions.getReadOnlyPacket();
        SerializeOptions serializeOptions2 = this.options;
        if (readOnlyPacket) {
            if (serializeOptions2.getOmitPacketWrapper() | this.options.getIncludeThumbnailPad()) {
                throw new XMPException("Inconsistent options for read-only packet", 103);
            }
            this.padding = 0;
        } else if (serializeOptions2.getOmitPacketWrapper()) {
    ...
    private void declareNamespace(String prefix, String namespace, Set<String> usedPrefixes, int indent) throws IOException {
        ...
        if (!prefix.isEmpty() && !usedPrefixes.contains(prefix)) {
            writeNewline();
            writeIndent(indent);
            write("xmlns:");
            write(prefix);
    ...
    private void serializeCanonicalRDFProperty(XMPNode node, boolean useCanonicalRDF, boolean emitAsRDFValue, int indent) throws IOException, XMPException {
        ...
        if (hasGeneralQualifiers && !emitAsRDFValue) {
            ...
            writeNewline();
            serializeCanonicalRDFProperty(node, useCanonicalRDF, true, indent2 + 1);
            Iterator it2 = node.iterateQualifier();
            while (it2.hasNext()) {
                XMPNode qualifier2 = (XMPNode) it2.next();
                if (!RDF_ATTR_QUALIFIER.contains(qualifier2.getName())) {
                    serializeCanonicalRDFProperty(qualifier2, useCanonicalRDF, false, indent2 + 1);
                }
            }
            if (useCanonicalRDF) {
                writeIndent(indent2);
            ...
            write(62);
            writeNewline();
            emitRDFArrayTag(node, true, indent2 + 1);
            if (node.getOptions().isArrayAltText()) {
                XMPNodeUtils.normalizeLangArray(node);
            }
            Iterator it3 = node.iterateChildren();
            while (it3.hasNext()) {
                serializeCanonicalRDFProperty((XMPNode) it3.next(), useCanonicalRDF, false, indent2 + 2);
            }
            emitRDFArrayTag(node, false, indent2 + 1);
            ...
            if (!node.hasChildren()) {
                ...
                while (it4.hasNext()) {
                    serializeCanonicalRDFProperty((XMPNode) it4.next(), useCanonicalRDF, false, indent2 + 1);
                }
                if (useCanonicalRDF) {
                    writeIndent(indent2);
                    write("</rdf:Description>");
