...
public final class Cam16 {
    ...
    public static Cam16 fromXyzInViewingConditions(double x, double y, double z, ViewingConditions viewingConditions) {
        ...
        if (atanDegrees < AudioStats.AUDIO_AMPLITUDE_NONE) {
    ...
    public static Cam16 fromUcsInViewingConditions(double jstar, double astar, double bstar, ViewingConditions viewingConditions) {
        ...
        if (h2 >= AudioStats.AUDIO_AMPLITUDE_NONE) {
            h = h2;
        } else {
            h = h2 + 360.0d;
        }
    ...
    public double[] xyzInViewingConditions(ViewingConditions viewingConditions, double[] returnArray) {
        double alpha = (getChroma() == AudioStats.AUDIO_AMPLITUDE_NONE || getJ() == AudioStats.AUDIO_AMPLITUDE_NONE) ? 0.0d : getChroma() / Math.sqrt(getJ() / 100.0d);
        ...
        double rCBase = Math.max((double) AudioStats.AUDIO_AMPLITUDE_NONE, alpha2);
        double rC = Math.signum(rA) * (100.0d / viewingConditions.getFl()) * Math.pow(rCBase, 2.380952380952381d);
        double gCBase = Math.max((double) AudioStats.AUDIO_AMPLITUDE_NONE, (Math.abs(gA) * 27.13d) / (400.0d - Math.abs(gA)));
        ...
        double bCBase = Math.max((double) AudioStats.AUDIO_AMPLITUDE_NONE, gC2);
