...
public final class DynamicColor {
    ...
    public double getTone(DynamicScheme scheme) {
        ...
        double fTone;
        double nContrast;
        boolean decreasingContrast = scheme.contrastLevel < AudioStats.AUDIO_AMPLITUDE_NONE;
        if (this.toneDeltaPair != null) {
            ...
            if ((fTone - nTone) * expansionDir >= delta) {
                ...
                double fTone2 = MathUtils.clampDouble(AudioStats.AUDIO_AMPLITUDE_NONE, 100.0d, nTone + (delta * expansionDir));
                if ((fTone2 - nTone) * expansionDir >= delta) {
                    nContrast = fTone2;
                } else {
                    nTone = MathUtils.clampDouble(AudioStats.AUDIO_AMPLITUDE_NONE, 100.0d, fTone2 - (delta * expansionDir));
                    nContrast = fTone2;
                }
            }
            if (50.0d <= nTone && nTone < 60.0d) {
                if (expansionDir > AudioStats.AUDIO_AMPLITUDE_NONE) {
                    nTone = 60.0d;
            ...
            } else if (50.0d <= nContrast && nContrast < 60.0d) {
                if (stayTogether) {
                    if (expansionDir > AudioStats.AUDIO_AMPLITUDE_NONE) {
                        nTone = 60.0d;
                        nContrast = Math.max(nContrast, 60.0d + (delta * expansionDir));
                ...
                } else if (expansionDir > AudioStats.AUDIO_AMPLITUDE_NONE) {
                    nContrast = 60.0d;
                } else {
                    nContrast = 49.0d;
                }
        ...
        if (this.secondBackground != null) {
            ...
            if (prefersLight) {
                ...
                return darkOption == -1.0d ? AudioStats.AUDIO_AMPLITUDE_NONE : darkOption;
            }
        }
        return answer;
    }
