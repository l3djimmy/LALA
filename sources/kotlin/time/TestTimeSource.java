...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m8796plusAssignLRDsOJo(long j) {
        long longDelta = Duration.m8726toLongimpl(j, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m8795overflowLRDsOJo(j);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m8690divUwyO8pc(j, 2);
        long $this$isSaturated$iv = Duration.m8726toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m8796plusAssignLRDsOJo(half);
                m8796plusAssignLRDsOJo(Duration.m8715minusLRDsOJo(j, half));
                return;
    ...
    private final void m8795overflowLRDsOJo(long j) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m8727toStringimpl(j)) + '.');
    }
}
